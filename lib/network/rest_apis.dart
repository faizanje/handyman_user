...
Future<ConfigurationResponse> getAppConfigurations({bool isCurrentLocation = false, double? lat, double? long}) async {
  try {
    ...
    if (data.paymentSettings != null) {
      setValue(PAYMENT_LIST, PaymentSetting.encode(data.paymentSettings.validate()));
    }
...
Future<DashboardResponse> userDashboard({bool isCurrentLocation = false, double? lat, double? long}) async {
  ...
  try {
    ...
    _dashboardResponse = DashboardResponse.fromJson(await handleResponse(await buildHttpResponse(endPoint, method: HttpMethodType.GET)));
    ...
    if (_dashboardResponse.privacyPolicy != null && _dashboardResponse.privacyPolicy!.value != null) {
      appStore.setPrivacyPolicy(_dashboardResponse.privacyPolicy!.value.validate());
    } else {
      appStore.setPrivacyPolicy(PRIVACY_POLICY_URL);
    }

    if (_dashboardResponse.termConditions != null && _dashboardResponse.termConditions!.value != null) {
    ...
    if (_dashboardResponse.generalSetting != null) {
      setValue(SITE_DESCRIPTION, _dashboardResponse.generalSetting!.siteDescription.validate());
      setValue(SITE_COPYRIGHT, _dashboardResponse.generalSetting!.siteCopyright.validate());
      setValue(FACEBOOK_URL, _dashboardResponse.generalSetting!.facebookUrl.validate());
      setValue(INSTAGRAM_URL, _dashboardResponse.generalSetting!.instagramUrl.validate());
      setValue(TWITTER_URL, _dashboardResponse.generalSetting!.twitterUrl.validate());
      setValue(LINKEDIN_URL, _dashboardResponse.generalSetting!.linkedinUrl.validate());
      setValue(YOUTUBE_URL, _dashboardResponse.generalSetting!.youtubeUrl.validate());

      appStore.setInquiryEmail(_dashboardResponse.generalSetting!.inquriyEmail.validate(value: INQUIRY_SUPPORT_EMAIL));

      appStore.setHelplineNumber(_dashboardResponse.generalSetting!.helplineNumber.validate(value: HELP_LINE_NUMBER));
    }

    setValue(IS_ADVANCE_PAYMENT_ALLOWED, _dashboardResponse.isAdvancedPaymentAllowed == '1');
    appStore.setEnableUserWallet(_dashboardResponse.enableUserWallet.validate());

    appStore.setUnreadCount(_dashboardResponse.notificationUnreadCount.validate());
