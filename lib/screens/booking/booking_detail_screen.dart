...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Widget buildTimeWidget({required BookingData bookingDetail}) {
    ...
    return Text(TimeOfDay(hour: bookingDetail.bookingSlot.validate().splitBefore(':').split(":").first.toInt(), minute: bookingDetail.bookingSlot.validate().splitBefore(':').split(":").last.toInt()).format(context),
  ...
  Widget customerReviewWidget({required List<RatingData> ratingList, required RatingData? customerReview, required BookingData bookingDetail}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (bookingDetail.status == BookingStatusKeys.complete && (bookingDetail.isFreeService || bookingDetail.paymentStatus == SERVICE_PAYMENT_STATUS_PAID))
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              24.height,
